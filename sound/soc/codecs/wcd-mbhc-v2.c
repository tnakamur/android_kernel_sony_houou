		...
		if (mbhc->impedance_detect &&
			...
			if (!skip_impdet_retry) {
				...
				pr_debug("%s: impedance L:%d R:%d\n", __func__,
					 mbhc->zl, mbhc->zr);
				WCD_MBHC_REG_READ(WCD_MBHC_MICB_VAL, val);
				pr_debug("%s: micbias val:%x\n", __func__,val);
				if (((mbhc->zl > 10000) || (mbhc->zr > 10000)) &&
					val == 0xC0 &&
					mbhc->micbias_enable &&
					jack_type == SND_JACK_HEADSET ) {
					pr_debug("%s: fall back to extension cable \n", __func__);
					mbhc->micbias_enable = false;
					mbhc->current_plug = MBHC_PLUG_TYPE_HIGH_HPH;
					mbhc->hph_status = 0;
					...
					if (mbhc->mbhc_cb->mbhc_common_micb_ctrl)
							mbhc->mbhc_cb->mbhc_common_micb_ctrl(codec,
								MBHC_COMMON_MICB_PRECHARGE,false);
...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	/* Check impedance value against jack corrosion issue */
	if (is_spl_hs && ((mbhc->zl > 10000) || (mbhc->zr > 10000))) {
		pr_debug("%s: Spl detection falls back to false\n",
					__func__);
		is_spl_hs = false;
	}
...
report:
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADSET &&
		mbhc->current_plug == MBHC_PLUG_TYPE_HIGH_HPH) {
		pr_debug("%s: change plug_type to high_hph if fallback\n", __func__);
		plug_type = MBHC_PLUG_TYPE_HIGH_HPH;
	}

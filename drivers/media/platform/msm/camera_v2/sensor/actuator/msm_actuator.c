...
static ssize_t msm_actuator_hall_pos_show(
	struct device *dev,
...
static DEVICE_ATTR(current_len_pos, 0644, msm_actuator_hall_pos_show, NULL);

static int32_t msm_actuator_register_hall_pos_device(
	struct msm_actuator_ctrl_t *o_ctrl,
	int _id)
{
	...
	CDBG("Enter\n");
	if (hall_pos_class_created == 0) {
		hall_pos_class = class_create(THIS_MODULE, "actuator");
		if (IS_ERR(hall_pos_class)) {
			int ret = PTR_ERR(hall_pos_class);

			CDBG("Unable to create class, err = %d\n", ret);
			return ret;
		}
		hall_pos_class_created = 1;
	}

	snprintf(hall_name, sizeof(hall_name), "hall%d", _id);
	hall_device = device_create(hall_pos_class, NULL, _id,
		(void *)o_ctrl, hall_name);

	if (hall_device == NULL)
		return -EIO;

	device_create_file(hall_device, &dev_attr_current_len_pos);

...
static int can_swap(gfp_t mask, int atomic_threshold, int movable_threshold)
{
	...
	zref = first_zones_zonelist(zonelist, high_zoneidx, NULL, &ziter);
...
static void calc_params(struct calculated_params *cp, gfp_t mask)
{
	...
	long irp = global_page_state(NR_INDIRECTLY_RECLAIMABLE_BYTES)
	  >> PAGE_SHIFT;
...
void print_obituary(struct task_struct *doomed,
	...
	lowmem_print(1, "Killing '%s' (%d), adj %hd,\n"
		     ...
		     (long)(PAGE_SIZE / 1024),
		     gfp_mask,
		     global_page_state(NR_INDIRECTLY_RECLAIMABLE_BYTES)
		     / 1024,
